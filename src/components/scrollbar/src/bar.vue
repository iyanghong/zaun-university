<template>
  <div
    :class="['t-scrollbar--bar', vertical ? 'is-vertical' : 'is-horizontal']"
  >
    <div
      class="t-scrollbar--thumb"
      ref="thumb"
      :style="getGetThumbStyle()"
      @mousedown="handleThumb"
    ></div>
  </div>
</template>

<script>
import { EventListener } from "@/utils/dom.js";
...
export default {
  ...
  methods: {
    ...
    handleThumb(e) {
      ...
      this._Mousemove = EventListener.listen(
        document,
        "mousemove",
        this.handleMouseMove
      );
      this._Mouseup = EventListener.listen(
        document,
        "mouseup",
        this.handleMouseUp
      );
