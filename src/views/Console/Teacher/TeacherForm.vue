...
import {addTeacher, getTeacher, updateTeacher} from "@/api/user";
...
export default {
    ...
    return {
      ...
      form: {
        ...
        rows: [
          ...
          {
            field: 'job', label: '职位', type: 'select', data: jobsData
          }
        ]
      }
    }
  ...
  methods: {
      ...
      generatePersonal().then(response => {
        if (response.success === true) {
          ...
          let data = {
            ...
            job: jobsData[parseInt(Math.random() * jobsData.length - 1)].value,
          };
    ...
    async handleLoadData() {
      let data = {};
      await getTeacher(this.data.id).then(response => {
        if (response.success === true) {
          data = response.data;
        }
      })
    ...
    handleUpdate(data) {
      ...
      this.loading = true;
      updateTeacher(postData).then(response => {
        if (response.success === true) {
